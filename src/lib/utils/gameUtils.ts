import { playerStore, enemiesStore, gameBoardStore, type Building, type Enemy, type Player } from "$lib/Stores/localstorage";

export function addEnemy(name: string, position: { x: number; y: number }) {
...
export function placeEntityOnBoard(entity: Enemy | Player | Building, x: number, y: number) {
  gameBoardStore.update((board) => {
    board.cells[y][x].content = entity.type as 'empty' | 'player' | 'enemy' | 'building';
...
export function movePlayerToBoard(player: Player, x: number, y: number) {
  gameBoardStore.update((board) => {
    // Удаление игрока с предыдущей клетки
    ...
    // Размещение игрока на новой клетке
